...
def weight_revise(original_score):

    device = (torch.device('cuda')
              if original_score.is_cuda
              else torch.device('cpu'))
    bsz = original_score.shape[0]

    init_score = np.array([0.3, 0.7])
    ...
    lamda = 0.8
    combine = (1 - lamda) * original_score + lamda * guide_score

    new_score = F.normalize(combine, p=1, dim=1).float()#.type(torch.float32)
