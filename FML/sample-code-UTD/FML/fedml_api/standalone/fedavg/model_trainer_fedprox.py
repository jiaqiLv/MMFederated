    ...
    def train(self, 
        ...
        for epoch in tqdm(range(epochs)):
            ...
            for x1, x2, y in train_data:
                ...
                proximal_term = torch.tensor(0.0,device=args.device)
            ...
            if client_idx != None:
                logging.info('Client Index = {}\tEpoch: {}\tLoss: {:.6f}\tLabeld Loss: {:.6f}\tUnlabeled Loss: {:.6f}'.format(
                client_idx, epoch, sum(epoch_loss) / len(epoch_loss), 
                sum(epoch_loss_labeled) / len(epoch_loss_labeled), 
                sum(epoch_loss_unlabeled) / len(epoch_loss_unlabeled)))
            else:
                logging.info('Epoch: {}\tLoss: {:.6f}\tLabeld Loss: {:.6f}\tUnlabeled Loss: {:.6f}'.format(
